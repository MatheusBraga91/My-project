...
void OnSkillButtonClicked(int skillIndex)
{
    ...
    void UseSkill(CharacterStats attacker, CharacterStats defender, Skill skill)
    {
        ...
        int damage = attacker.currentPower + skill.power - defender.currentDefense; // Example calculation
...
void UpdateHealthUI()
{
    // Smoothly update Player Health Bar
    if (playerHealthCoroutine != null)
    {
        StopCoroutine(playerHealthCoroutine);
    }
    playerHealthCoroutine = StartCoroutine(SmoothHealthBarUpdate(playerHealthBar, playerStats.currentHealth, playerStats.baseHealth));

    // Smoothly update Enemy Health Bar
    if (enemyHealthCoroutine != null)
    {
        StopCoroutine(enemyHealthCoroutine);
    }
    enemyHealthCoroutine = StartCoroutine(SmoothHealthBarUpdate(enemyHealthBar, enemyStats.currentHealth, enemyStats.baseHealth));
...
private IEnumerator SmoothHealthBarUpdate(Image healthBar, int currentHealth, int maxHealth)
{
    float targetFillAmount = (float)currentHealth / maxHealth;
    float initialFillAmount = healthBar.fillAmount;
    float elapsedTime = 0f;
    float duration = 0.5f; // Adjust the duration as needed for the smoothness

    while (elapsedTime < duration)
    {
        elapsedTime += Time.deltaTime;
        healthBar.fillAmount = Mathf.Lerp(initialFillAmount, targetFillAmount, elapsedTime / duration);
        yield return null;
    }

    healthBar.fillAmount = targetFillAmount; // Ensure it ends exactly at the target value
}
}
